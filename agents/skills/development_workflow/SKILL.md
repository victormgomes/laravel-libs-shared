...
on the host if they require specific environments. Instead, always use the Docker
container `dev`.
...
- **Releases & Commits:** We use Release Please or Semantic Release for
  automated semantic versioning. You **must** use Conventional Commits (e.g.,
  `feat:`, `fix:`). To trigger a major version bump for breaking changes, you
  **must** append an exclamation mark `!` to the commit type (e.g.,
  `refactor!:` or `feat!:`).
